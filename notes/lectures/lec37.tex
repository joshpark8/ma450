...
\begin{example}
  Let \defmap{\phi}{\Z[x]}{\Z}{f(x)}{f(0)} be a ring homomorphism. Then \( \kerphi = \cyc{x} \). By Thm 15.3, \( \fr{\Z[x]}{\cyc{x}}\iso\Z \). Since \( \Z \) is an integral domain but not a field, \( \cyc{x} \) is a prime but not maximal in \( \Z[x] \).
\end{example}
