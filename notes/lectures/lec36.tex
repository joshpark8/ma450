...
\begin{theorem}
  Let \( R \) be a commutative ring with 1. Let \( A \) be an ideal of \( R \). Then,
  \begin{enumerate}
    \item \( \qg{R}{A} \) = integral domain \( \iff A =  \) prime
    \item \( \qg{R}{A} \) = field \( \iff A =  \) maximal
  \end{enumerate}
\end{theorem}

\begin{proof}
  \begin{enumerate}
    \item \( \qg{R}{A} = \) integral domain
    ...
    \item (\( \imp \)) Suppose \( \qg{R}{A} = \) field. Let \( B\supset A \) be an ideal (\( B\neq A \)). Then \( \exists b \in B \) such that \( b\not\in A \)
    ...
    Consider \( B = \{rb+a \st r\in R, a\in A\} \) (check \( B \) is an ideal and \( B\supset A,\ B\neq A \))
