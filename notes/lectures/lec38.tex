...
\begin{definition}[Ring of Polynomials over \( R \)]
  ...
  \end{align*}
  is called the \tun{ring of polynomials over \( R \) in the indeterminate \( x \).}
\end{definition}
...
\end{align*}
If \( a_n\neq 0 \), then \tun{\( \deg(f) = n \)} and \( a_n \) is called the \tun{leading coefficient} of \( f \).

If \( a_n\neq 0 \) is the multiplicative identity of \( R \), then \( f \) is called a \tun{monic} polynomial.

\( a_0 \) is called the \tun{constant term} of \( f \).

If \( f(x) = a_0 \) then \( f \) is called a \tun{constant polynomial}.
...
\begin{example}
  \( x\sq+3x+2 \) in \( \Zsi[x] \) has \tun{four} zeros in \( \Zsi \) (1, 2, 4, 5).
\end{example}

\begin{definition}[Principal Ideal Domain (PID)]
  A \tun{principal ideal domain (PID)} is an integral domain \( R \) such that every ideal has the form \( \cyc{a}=\{ra\st r\in R\} \) for some \( a\in R \)
\end{definition}
