...
\bd{a.  \Zei \qquad b.  \ZZ{10} \qquad c.  \ZZ{12} \qquad d. }\Zn
...
\begin{enumerate}[label=\bd{\alph*.}]
  ...
  \item \( \pidl{a} = \pidl{m} + \pidl{n} \) \\
    ...
    Suppose \( m \) and \( n \) are \ital{not} relatively prime. That is, \( \gcd(m,n) = d > 1\). So any element \( \gamma\in\pidl{m}+\pidl{n} \) must be of the form \( \gamma=m\alpha+n\beta \). Notice that \begin{align*}
